/**
 * 移动端 - 故障报告提交页面逻辑
 * URL 格式: /mobile/fault.html?machine_id=1
 */
;(function () {
  'use strict';
  ...
  form.addEventListener('submit', async function (e) {
    ...
    try {
      var result = await API.faults.create(data);
      showToast('故障报告已提交！记录ID: ' + result.id, 'success');
      resetForm();
    } catch (err) {
      showToast('提交失败: ' + err.message, 'error');
    } finally {
      submitBtn.disabled = false;
      submitBtn.textContent = '提交故障报告';
    }
  });
  ...
  function collectFormData() {
    ...
    var faultType = getValue('fault_type');
    var description = getValue('description');

    if (!operatorName) {
      showToast('请填写上报人员', 'warning');
      focusField('operator_name');
      return null;
    }
    if (!faultType) {
      showToast('请选择故障类型', 'warning');
      focusField('fault_type');
      return null;
    }
    if (!description) {
      showToast('请填写故障描述', 'warning');
  ...
  function resetForm() {
    form.reset();
    // 重置严重程度为默认值
    document.getElementById('severity').value = '一般';
  }
