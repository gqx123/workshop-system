(function () {
    ...
    function open(record) {
      ...
      if (record.details && record.details.length) {
        for (var i = 0; i < record.details.length; i++) {
          ...
          var noteHtml = d.note ? '<span class="detail-note">(' + esc(d.note) + ')</span>' : '';
          var photoHtml = '';
          if (d.photo) {
            photoHtml = '<img class="detail-photo" src="/api/inspection-photos/' + d.photo + '?t=' + Date.now() + '" onclick="window.open(this.src)" title="点击查看大图">';
          }
          html += '<div class="insp-detail-item">';
          html += '  <span class="detail-name">' + esc(d.item_name) + '</span>';
          html += '  ' + photoHtml;
          html += '  <span class="tag ' + tagCls + '">' + esc(d.result) + '</span>';
          html += '  ' + noteHtml;
          html += '</div>';
        }
      } else {
      ...
      if (record.remark) {
        html += '<div style="margin-top:12px;padding:10px;background:var(--surface-2);border-radius:var(--radius-sm);font-size:13px;color:var(--text-muted);">备注：' + esc(record.remark) + '</div>';
      }
      bodyEl.innerHTML = html;
      overlay.classList.add('open');
    }
  ...
  (function () {
    ...
    var newRequirePhotoCb = document.getElementById('tplNewRequirePhoto');
    ...
    function renderList(items) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        var rpFlag = item.requires_photo ? 1 : 0;
        html += '<div class="tpl-item">';
        html += '<span class="tpl-order">' + (i + 1) + '</span>';
        html += '<span class="tpl-name">' + esc(item.item_name);
        if (item.requires_photo) {
          html += ' <span class="tag tag-primary" style="font-size:10px;padding:1px 5px;margin-left:6px;">需拍照</span>';
        }
        html += '</span>';
        html += '<span class="tpl-actions">';
        html += '<button class="btn btn-ghost" data-action="toggle-photo" data-id="' + item.id + '" data-requires="' + rpFlag + '" style="font-size:11px;padding:3px 8px;">' + (item.requires_photo ? '取消拍照' : '需拍照') + '</button>';
    ...
    function handleItemClick(e) {
      ...
      if (action === 'edit') {
        ...
        var current = parseInt(btn.getAttribute('data-requires'), 10);
        var newVal = current ? 0 : 1;
        API.inspectionTemplates.update(id, { requires_photo: newVal })
          .then(function () { loadTemplates(); window.showToast(newVal ? '已设为需拍照' : '已取消需拍照', 'success'); })
          .catch(function (err) { window.showToast('更新失败: ' + err.message, 'error'); });
    ...
    addBtn.addEventListener('click', function () {
      ...
      var requirePhoto = newRequirePhotoCb.checked ? 1 : 0;
      API.inspectionTemplates.create({ machine_id: currentMachineId, item_name: name, requires_photo: requirePhoto })
        .then(function () { newNameInput.value = ''; newRequirePhotoCb.checked = false; loadTemplates(); window.showToast('添加成功', 'success'); })
    ...
    importConfirm.addEventListener('click', function () {
      ...
      API.inspectionTemplates.importCSV(currentMachineId, csvItems)
        ...
        .catch(function (err) { window.showToast('导入失败: ' + err.message, 'error'); });
    });
